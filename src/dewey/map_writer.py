...
def write_map(points: list[MapPoint], output: Path, title: str) -> None:
    ...
    clusters: defaultdict[int, list[MapPoint]] = defaultdict(list)
    for point in points:
        clusters[point.cluster_id].append(point)

    traces = [cluster_trace(members) for cluster_id, members in clusters.items() if cluster_id != NOISE]
    if NOISE in clusters:
        traces.insert(0, cluster_trace(clusters[NOISE]))
    if dimensions == {2}:
        traces.append(label_trace([members for cluster_id, members in clusters.items() if cluster_id != NOISE]))

    figure = go.Figure(data=traces)
    figure.update_layout(title=title, legend_title_text="Cluster", hovermode="closest", template="plotly_white")
    figure.write_html(str(output), include_plotlyjs=True, post_script=OPEN_ON_CLICK)


def cluster_trace(members: list[MapPoint]) -> go.BaseTraceType:
    unclustered = members[0].cluster_id == NOISE
    hover = [
    ...
    return trace(
        members,
        name=html.escape(members[0].cluster),
        mode="markers",
        text=hover,
...
def label_trace(clusters: list[list[MapPoint]]) -> go.BaseTraceType:
    centers = [np.median([point.coordinates for point in members], axis=0) for members in clusters]

    return go.Scatter(
        x=[center[0] for center in centers],
        y=[center[1] for center in centers],
        mode="text",
        text=[html.escape(members[0].cluster) for members in clusters],
